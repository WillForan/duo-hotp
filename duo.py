...
def save_secret(hotp_secret, count):
    ...
    secrets = {"hotp_secret": hotp_secret, "count": count}
...
def HOTP():
    ...
    hotp = pyotp.HOTP(
        encoded_secret
    )  # As long as the secret key is the same, the HOTP object is the same
